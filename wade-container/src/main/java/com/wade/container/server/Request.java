...
package com.wade.container.server;
...
import com.wade.container.http.HttpCookie;
import com.wade.container.http.HttpFields;
import com.wade.container.http.HttpHeaders;
import com.wade.container.http.HttpMethods;
import com.wade.container.http.HttpParser;
import com.wade.container.http.HttpStatus;
import com.wade.container.http.HttpURI;
import com.wade.container.http.HttpVersions;
import com.wade.container.http.MimeTypes;
import com.wade.container.io.Buffer;
import com.wade.container.io.BufferUtil;
import com.wade.container.io.EndPoint;
import com.wade.container.io.nio.DirectNIOBuffer;
import com.wade.container.io.nio.IndirectNIOBuffer;
import com.wade.container.io.nio.NIOBuffer;
import com.wade.container.server.handler.ContextHandler;
import com.wade.container.server.handler.ContextHandler.Context;
import com.wade.container.util.Attributes;
import com.wade.container.util.AttributesMap;
...
import com.wade.container.util.UrlEncoded;
 ...
 * {@link server.session.SessionHandler} which checks for session cookies and enables the ability to create new sessions.</li>
 *
 * <li>The {@link Request#getServletPath()} method will return null until the request has been passed to a <code>servlet.ServletHandler</code>
 ...
 * {@link ContextHandler#getMaxFormContentSize()} or if there is no context then the "com.wade.container.server.Request.maxFormContentSize" {@link Server}
 ...
 * "com.wade.container.server.Request.maxFormKeys" {@link Server} attribute.
 *
 *
 */
public class Request implements HttpServletRequest
{
                        ...
                        && (HttpMethods.POST.equals(getMethod()) || HttpMethods.PUT.equals(getMethod())))
                {
                    ...
                    if (content_length != 0)
                    {
                        try
                        {
                            ...
                            {
                                Object obj = _connection.getConnector().getServer().getAttribute("com.wade.container.server.Request.maxFormContentSize");
                                if (obj == null)
                            ...
                            if (maxFormKeys < 0)
                            {
                                Object obj = _connection.getConnector().getServer().getAttribute("com.wade.container.server.Request.maxFormKeys");
                                if (obj == null)
                                    maxFormKeys = 1000;
                                else if (obj instanceof Number)
                                {
    ...
    {
        if ("com.wade.container.io.EndPoint.maxIdleTime".equalsIgnoreCase(name))
    ...
    /* ------------------------------------------------------------ */
    /*
     * Set a request attribute. if the attribute name is "com.wade.container.server.server.Request.queryEncoding" then the value is also passed in a call to
     * {@link #setQueryEncoding}. <p> if the attribute name is "com.wade.container.server.server.ResponseBuffer", then the response buffer is flushed with @{link
     * #flushResponseBuffer} <p> if the attribute name is "com.wade.container.io.EndPoint.maxIdleTime", then the value is passed to the associated {@link
    ...
    public void setAttribute(String name, Object value)
    {
        ...
        if (name.startsWith("com.wade.container."))
        {
            if ("com.wade.container.server.Request.queryEncoding".equals(name))
                setQueryEncoding(value == null?null:value.toString());
            else if ("com.wade.container.server.sendContent".equals(name))
            {
                try
                {
            ...
            else if ("com.wade.container.server.ResponseBuffer".equals(name))
            {
                try
                {
            ...
            else if ("com.wade.container.io.EndPoint.maxIdleTime".equalsIgnoreCase(name))
            {
                try
                {
     ...
     *         {@link #setContext(server.handler.ContextHandler.Context)} call.
     */
     ...
     * geParameter methods.
     *
     * The request attribute "com.wade.container.server.server.Request.queryEncoding" may be set as an alternate method of calling setQueryEncoding.
     *
     * @param queryEncoding
     */
