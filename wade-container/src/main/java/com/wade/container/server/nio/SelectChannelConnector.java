...
package com.wade.container.server.nio;
...
import com.wade.container.io.nio.SelectChannelEndPoint;
import com.wade.container.io.nio.SelectorManager;
import com.wade.container.io.nio.SelectorManager.SelectSet;
import com.wade.container.server.AsyncHttpConnection;
 ...
 * model. Direct NIO buffers are used and threads are only allocated to
 * connections with requests. Synchronization is used to simulate blocking for
 * the servlet API, and any unflushed content at the end of request handling is
 * written asynchronously.
 * </p>
 * <p>
 * This connector is best used when there are a many connections that have idle
 * periods.
 * </p>
 * <p>
 * When used with {@link continuation.Continuation},
 * threadless waits are supported. If a filter or servlet returns after calling
 * {@link Continuation#suspend()} or when a runtime exception is thrown from a
 * call to {@link Continuation#undispatch()}, Jetty will will not send a
 * response to the client. Instead the thread is released and the Continuation
 * is placed on the timer queue. If the Continuation timeout expires, or it's
 * resume method is called, then the request is again allocated a thread and the
 * request is retried. The limitation of this approach is that request content
 * is not available on the retried request, thus if possible it should be read
 * after the continuation or saved as a request attribute or as the associated
 * object of the Continuation instance.
 * </p>
 * 
 * @org.apache.xbean.XBean element="nioConnector"
 *                         description="Creates an NIO based socket connector"
 */
public class SelectChannelConnector extends AbstractNIOConnector {
	...
	/**
	 * Set the number of connections, which if exceeded places this manager in
	 * low resources state. This is not an exact measure as the connection count
	 * is averaged over the select sets.
	 * 
	 * @param lowResourcesConnections
	...
	/**
	 * Set the period in ms that a connection is allowed to be idle when this
	 * there are more than {@link #getLowResourcesConnections()} connections.
	 * This allows the server to rapidly close idle connections in order to
	 * gracefully handle high load situations.
	 * 
	 * @param lowResourcesMaxIdleTime
	 *            the period in ms that a connection is allowed to be idle when
	 *            resources are low.
	...
	/*
	 * @see
	 * com.wade.container.server.server.AbstractConnector#
	 * doStart()
	 */
	@Override
