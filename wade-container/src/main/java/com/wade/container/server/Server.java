...
import com.wade.container.http.HttpGenerator;
...
import com.wade.container.util.component.Container;
import com.wade.container.util.component.Destroyable;
...
import com.wade.container.util.thread.ShutdownThread;
import com.wade.container.util.thread.ThreadPool;
...
public class Server extends HandlerWrapper implements Attributes {
	...
	static {
		...
		if (Server.class.getPackage() != null && Server.class.getPackage().getImplementationVersion() != null)
			...
			__version = System.getProperty("wade.container.version", "1.x.y-SNAPSHOT");
	}
	...
	private boolean _sendServerVersion = false; // send Server: header
	 ...
	 * {@link SelectChannelConnector} at the passed port.
	 */
	...
	/**
	 * Convenience constructor Creates server and a
	 * {@link SelectChannelConnector} at the passed address.
	 */
	...
	public void setStopAtShutdown(boolean stop) {
		...
		if (stop) {
			...
			if (!_stopAtShutdown) {
				// only register to stop if we're already started (otherwise
				// we'll do it in doStart())
				if (isStarted())
	...
	/**
	 * Conveniance method which calls {@link #getConnectors()} and
	...
	/**
	 * @param dumpAfterStart
	 *            true if {@link #dumpStdErr()} is called after starting
	 */
	...
	/**
	 * @param dumpBeforeStop
	 *            true if {@link #dumpStdErr()} is called before stopping
	 */
	...
	protected void doStart() throws Exception {
		...
		LOG.info("WADE-Container-" + __version);
	...
	/*
	 * Handle a request from a connection. Called to handle a request on the
	 * connection when either the header has been received, or after the entire
	 * request has been received (for short requests of known length), or on the
	 * dispatch of an async request.
	 */
	...
	public void handleAsync(AbstractHttpConnection connection) throws IOException, ServletException {
		...
		if (path != null) {
			...
			if (uri.getQuery() != null)
				baseRequest.mergeQueryString(uri.getQuery()); // we have to
																// assume
																// dispatch path
																// and query are
																// UTF8
	...
	/**
	 * Add an associated bean. The bean will be added to the servers
	 * {@link Container} and if it is a {@link LifeCycle} instance, it will be
	...
	/*
	 * @see org.eclipse.util.AttributesMap#setAttribute(java.lang.String,
	 * java.lang.Object)
	 */
	...
	/**
	 * Set graceful shutdown timeout. If set, the internal <code>doStop()</code>
	 * method will not immediately stop the server. Instead, all
	 * {@link Connector}s will be closed so that new connections will not be
	 * accepted and all handlers that implement {@link Graceful} will be put
	 * into the shutdown mode so that no new requests will be accepted, but
	 * existing requests can complete. The server will then wait the configured
	...
	/*
	 * A handler that can be gracefully shutdown. Called by doStop if a {@link
	 * #setGracefulShutdown} period is set. TODO move this somewhere better
	 */
