...
import com.wade.container.server.Dispatcher;
...
import com.wade.container.util.component.Dumpable;
 ...
 *
 * <p>
 * If the context init parameter "com.wade.container.server.context.ManagedAttributes" is set to a comma separated list of names, then they are treated as
 ...
 * <p>
 * The maximum size of a form that can be processed by this context is controlled by the system properties server.Request.maxFormKeys
 * and server.Request.maxFormContentSize.  These can also be configured with {@link #setMaxFormContentSize(int)} and {@link #setMaxFormKeys(int)}
 * 
...
public class ContextHandler extends ScopedHandler implements Attributes, Server.Graceful
{
     ...
     */
    public static final String MANAGED_ATTRIBUTES = "com.wade.container.server.context.ManagedAttributes";
    ...
    private int _maxFormKeys = Integer.getInteger("com.wade.container.server.Request.maxFormKeys",-1).intValue();
    private int _maxFormContentSize = Integer.getInteger("com.wade.container.server.Request.maxFormContentSize",-1).intValue();
    ...
    /* ------------------------------------------------------------ */
    /*
     * @see com.wade.container.org.eclipse.thread.AbstractLifeCycle#doStart()
     */
    @Override
     ...
     *
     * @see com.wade.container.server.handler.ContextHandler.Context
     */
    ...
    /* ------------------------------------------------------------ */
    /*
     * @see com.wade.container.org.eclipse.thread.AbstractLifeCycle#doStop()
     */
    @Override
    ...
    /* ------------------------------------------------------------ */
    /*
     * @see com.wade.container.server.Handler#handle(javax.servlet.http.HttpServletRequest, javax.servlet.http.HttpServletResponse)
     */
    ...
    /* ------------------------------------------------------------ */
    /**
     * @see com.wade.container.server.handler.ScopedHandler#doScope(java.lang.String, server.Request, javax.servlet.http.HttpServletRequest,
    ...
    private static final boolean isPermit(HttpServletRequest request) {
    	
    	/** 负载均衡发起的探针请求，不会携带X-Forwarded-For头 */
    	String uri = request.getRequestURI();
		if (uri.endsWith("probe.jsp")) {
			return true;
		}
    	
		if (SourceAddressControl.isXffCheckEnable()) {
			String xff = request.getHeader("X-Forwarded-For");
			if (!SourceAddressControl.isXffPermit(xff)) { // 一定需要X-Forwarded-For
				LOG.warn("deny illegal access, x-forwarded-for:" + xff);
				return false;
			}
		}
    ...
    /* ------------------------------------------------------------ */
    /**
     * @see com.wade.container.server.handler.ScopedHandler#doHandle(java.lang.String, server.Request, javax.servlet.http.HttpServletRequest,
     *      javax.servlet.http.HttpServletResponse)
     */
    @Override
    ...
    {
    	if (!isPermit(request)) {
    		response.sendError(HttpServletResponse.SC_FORBIDDEN, "illegal access error!");
    		return;
    	}
    ...
    public class Context implements ServletContext
    {
        ...
        public String getServerInfo()
        {
        	...
        	return "wade-container/" + Server.getVersion();
        }
