...
package com.wade.container.util.thread;
...
import com.wade.container.util.BlockingArrayQueue;
...
import com.wade.container.util.thread.ThreadPool.SizedThreadPool;
...
public class QueuedThreadPool extends AbstractLifeCycle implements SizedThreadPool, Executor, Dumpable {
	...
	public QueuedThreadPool() {
		//_name = "jetty-" + super.hashCode();
		_name = "wade-container-" + super.hashCode();
	}
	...
	public boolean dispatch(Runnable job) {
		if (isRunning()) {
			...
			if (_jobs.offer(job)) {
				// If we had no idle threads or the jobQ is greater than the
				// idle threads
				if (idle == 0 || jobQ > idle) {
					int threads = _threadsStarted.get();
							...
							if (_maxIdleTimeMs <= 0)
								...
								if (size > _minThreads) {
									...
									if (last == 0 || (now - last) > _maxIdleTimeMs) {
										shrink = _lastShrink.compareAndSet(last, now) && _threadsStarted.compareAndSet(size, size - 1);
										if (shrink)
											return;
									}
								}
