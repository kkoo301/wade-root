...
package com.wade.container.util.log;

import java.io.PrintStream;
import java.security.AccessControlException;
import java.util.Properties;

import com.wade.container.util.DateCache;

/**
 * StdErr Logging. This implementation of the Logging facade sends all logs to StdErr with minimal formatting.
 * <p>
 * If the system property "com.wade.container.LEVEL" is set to one of the following (ALL, DEBUG, INFO, WARN), then set
 * the eclipse jetty root level logger level to that specified level. (Default level is INFO)
 * <p>
 * If the system property "com.wade.container.util.log.SOURCE" is set, then the source method/file of a log is logged.
 * For named debuggers, the system property name+".SOURCE" is checked, eg "com.wade.container.util.log.stderr.SOURCE". 
 * If it is not not set, then "com.wade.container.util.log.SOURCE" is used as the default.
 * <p>
 * If the system property "com.wade.container.util.log.stderr.LONG" is set, then the full, unabbreviated name of the logger is
 * used for logging.
 */
public class StdErrLog extends AbstractLogger
{
    ...
    private final static boolean __source = Boolean.parseBoolean(Log.__props.getProperty("com.wade.container.util.log.SOURCE",
            Log.__props.getProperty("com.wade.container.util.log.stderr.SOURCE","false")));
    private final static boolean __long = Boolean.parseBoolean(Log.__props.getProperty("com.wade.container.util.log.stderr.LONG","false"));
    ...
    static
    {
        ...
        { "DEBUG", "com.wade.container.util.log.DEBUG", "com.wade.container.util.log.stderr.DEBUG" };
        ...
        for (String deprecatedProp : deprecatedProperties)
        {
            ...
            {
                System.err.printf("System Property [%s] has been deprecated! (Use LEVEL=DEBUG instead)%n",deprecatedProp);
            }
        }
     ...
     * <pre>
     * Examples:
     * "com.wade.container.test.FooTest"           = "oejt.FooTest"
     * "com.wade.container.server.logging.LogTest" = "orjsl.LogTest"
     * </pre>
     *
     * @param classname
    ...
    private void tag(StringBuilder buffer, String d, int ms, String tag)
    {
        ...
        if (_source)
        {
            ...
            for (int i = 0; i < frames.length; i++)
            {
                ...
                if (!_printLongNames && clazz.startsWith("com.wade.container."))
                {
