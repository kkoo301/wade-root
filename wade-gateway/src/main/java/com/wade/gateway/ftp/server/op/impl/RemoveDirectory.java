 ...
 * @date: 2014-11-24
 */
public final class RemoveDirectory extends AbstractOperation {

	private static final Logger log = Logger.getLogger(RemoveDirectory.class);
	...
	public void execute(String siteId, Request baseRequest, HttpServletRequest request, HttpServletResponse response) throws IOException {
		...
		try {
			...
			if (!client.removeDirectory(dirPath)) {
				throw new Exception(RemoveDirectory.class.getName() + " 执行失败!");
			}
