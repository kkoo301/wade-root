...
/**
 * TimesTen PreparedStatement对象,getGeneratedKeys()未处理
 * 需验证getResultSet对象是否会存在不释放的情况
 * 需验证getConnection()是否为MySQLConnection实例
 * 
 * @className: TimesTenPreparedStatement.java
 * @author: liaosheng
 * @date: 2014-3-24
 */
public class TimesTenPreparedStatement implements PreparedStatement {

	private static final Logger log = Logger.getLogger(TimesTenPreparedStatement.class);
	
	private PreparedStatement stmt = null;
	private ConnectionWrapper proxyConnection = null;
	
	public TimesTenPreparedStatement(ConnectionWrapper proxyConnection, PreparedStatement stmt) throws SQLException {
		if (log.isDebugEnabled()) {
			log.debug("创建TimesTenPreparedStatement,queryTimeout=10.");
		}
