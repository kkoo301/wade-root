...
/**
 * MySQL PreparedStatement对象,getGeneratedKeys()未处理
 * 需验证getResultSet对象是否会存在不释放的情况
 * 需验证getConnection()是否为MySQLConnection实例
 * 
 * @className: MySQLPreparedStatement.java
 * @author: liaosheng
 * @date: 2014-3-24
 */
public class MySQLPreparedStatement implements PreparedStatement {
	
	private static final Logger log = Logger.getLogger(MySQLPreparedStatement.class);

	private PreparedStatement stmt = null;
	
	private ConnectionWrapper proxyConnection = null;
	
	public MySQLPreparedStatement(ConnectionWrapper proxyConnection, PreparedStatement stmt) throws SQLException {
		if (log.isDebugEnabled()) {
			log.debug(String.format("创建MySQLPreparedStatement,queryTimeout=%d.", proxyConnection.getStmtTimeout()));
		}
