...
import com.ailk.database.jdbc.altibase.AltibasePreparedStatement;
import com.ailk.database.jdbc.altibase.AltibaseResultSet;
import com.ailk.database.jdbc.wrapper.ConnectionWrapper;

/**
 * Altibase PreparedStatement对象,getGeneratedKeys()未处理
 * 需验证getResultSet对象是否会存在不释放的情况
 * 需验证getConnection()是否为MySQLConnection实例
 * 
 * @className: AltibasePreparedStatement.java
 * @author: liaosheng
 * @date: 2014-3-24
 */
public class AltibasePreparedStatement implements PreparedStatement {

	private static final Logger log = Logger.getLogger(AltibasePreparedStatement.class);
	
	private PreparedStatement stmt = null;
	private ConnectionWrapper proxyConnection = null;
	
	public AltibasePreparedStatement(ConnectionWrapper proxyConnection, PreparedStatement stmt) throws SQLException {
		if (log.isDebugEnabled()) {
			log.debug(String.format("创建AltibasePreparedStatement,queryTimeout=%d.", proxyConnection.getStmtTimeout()));
		}
	...
	public ResultSet executeQuery(String sql) throws SQLException {
		...
		return new AltibaseResultSet(rs);
	}
