...
/**
 * SQLitePreparedStatement对象,getGeneratedKeys()未处理
 * 需验证getResultSet对象是否会存在不释放的情况
 * 需验证getConnection()是否为SQLitePreparedStatement实例
 * 
 * @className: SQLitePreparedStatement.java
 * @author: liaosheng
 * @date: 2014-3-24
 */
public class SQLitePreparedStatement implements PreparedStatement {

	private static final Logger log = Logger.getLogger(SQLitePreparedStatement.class);
	
	private PreparedStatement stmt = null;
	private ConnectionWrapper proxyConnection = null;
	
	public SQLitePreparedStatement(ConnectionWrapper proxyConnection, PreparedStatement stmt) throws SQLException {
		if (log.isDebugEnabled()) {
			log.debug(String.format("创建SQLitePreparedStatement,queryTimeout=%d.", proxyConnection.getStmtTimeout()));
		}
