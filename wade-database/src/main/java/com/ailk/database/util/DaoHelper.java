...
public class DaoHelper {
		...
		if (DaoUtil.isDatetimeColumn(column.getColumnType())) {
			if (columnValue != null) {
				if ("".equals(columnValue)) {
					...
					columnValue = DaoUtil.encodeTimestamp(columnValue.toString());
				}
			}
		} else if (column.getColumnType() == Types.LONGVARCHAR) {
			if (columnValue != null && !"".equals(columnValue)) {
				columnValue = new StringReader(columnValue.toString());
			}
		}
		...
		while (rs.next() && max > 0) {
			...
			if (null == names) {
				...
				for (int i = 1; i <= size; i++) {
					String name = rsmd.getColumnName(i); // 数据库字段名统一约定为大写
	...
	public static Object callFunc(Connection conn, String name, String[] paramNames, IData params, int[] paramKinds,
		...
		if(returnType == oracle.jdbc.OracleTypes.CURSOR){
			ResultSet rs = (ResultSet)retval;
            ...
            String names[] = null;
            IData data;
            for(; rs.next(); list.add(data))
            {
                data = new DataMap(colcnt);
                if(null == names)
                {
                    int size = rsmd.getColumnCount();
                    names = new String[size];
                    ...
                    {
                        String columname = rsmd.getColumnLabel(j).toUpperCase();
                        names[j - 1] = columname;
                        data.put(columname, rs.getString(columname));
                    }

                    continue;
                }
                int j = 0;
                for(int size = names.length; j < size; j++)
                    data.put(names[j], rs.getString(names[j]));
