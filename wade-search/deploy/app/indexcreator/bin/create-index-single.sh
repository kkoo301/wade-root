...
[ "$#" == "0" ] && echo "Usage: ./create-index-single.sh SEARCH_CODE1 SEARCH_CODE2 ..." && exit 255
...
NOW=`date +%Y%m%d`
java -server $MEM_OPTS -cp $CLASSPATH $JAVA_OPTIONS com.ailk.search.server.index.IndexCreator $1 | tee -a $APP_HOME/log/create-index-single-${NOW}.log
