...
public class DataMap extends HashMap<String, Object> implements IData {
		...
		if (object != null) {
			...
			while (keys.hasNext()) {
				...
				if(((String)key).indexOf(CLASS_REP_STRING2)!=-1){
					String k = (String)key;
					key = k.replaceAll(CLASS_REP_STRING2, CLASS_STRING2);
				}
				
				if (value != null) {
					if (value instanceof JSONObject) {
						data.put(key, JSONObject.NULL.equals(value) ? null : DataMap.fromJSONObject((JSONObject) value));
					} else if (value instanceof JSONArray) {
						data.put((String) key, JSONObject.NULL.equals(value) ? null	: DatasetList.fromJSONArray((JSONArray) value));
	...
	public IDataset getDataset(String name, IDataset def) {
		...
		if (value instanceof IDataset) {
			...
			IDataset ds = DatasetList.fromJSONArray((JSONArray) value);
			return ds;
		} else {
			return def;
		}	
	...
	public String toString() {
		JSONObject map = new JSONObject(this);
		return map.toString();
	}
	...
	public static void main(String[] args) {
		...
		IData d2 = new DataMap(d.toString());
